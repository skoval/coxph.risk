...
```{r}
data <- rcox.exp(n=10000,lambda1=1,lambda2=.5)
...
```{r}
cox.exp(1,3,lambda1=1,lambda2=.5)
```
...
test <- data.frame(
    time = min.time, 
    disease = times[,1]==min.time,
    competing1 = times[,2]==min.time,
    competing2 = times[,3]==min.time,
    x1 = rnorm(nrow(times)),
    x2 = runif(nrow(times)),
    x3 = rbinom(n=nrow(times),size=1,p=.3)
    )
...
To demonstrate the usage of this function, we will compute predicted risks based on the `test` data set created in the example above. The `test` data set contains outcomes for three time-to-events, which each follow a Cox-Exp model, having the baseline hazard rates of `r lambdas[1]`, `r lambdas[2]`, and `r lambdas[3]` and three unassociated (noise) covariates `x1`, `x2` and `x3`. 
...
```{r}
fit1 <- survreg(Surv(time, disease)~x1+x2+x3
                ,dist = "exponential", data = test)

fit2 <- survreg(Surv(time, competing1)~x1+x2+x3
                ,dist = "exponential", data = test)

fit3 <- survreg(Surv(time, competing2)~x1+x2+x3
                ,dist = "exponential", data = test)
...
```{r}
coxph.risk(.2, 1, projection.data=temp, cox1, cox2, cox3)
coxph.risk(.4, 1, projection.data=temp, cox1, cox2, cox3)
coxph.risk(.6, 1, projection.data=temp, cox1, cox2, cox3)
```
